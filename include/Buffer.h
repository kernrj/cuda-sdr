...
class Buffer {
 public:
  ...
  Buffer(size_t capacity, size_t offset, size_t end) : mCapacity(capacity), mOffset(offset), mEnd(end) {}
   ...
   */
  [[nodiscard]] virtual std::shared_ptr<Buffer> slice(size_t offsetInOriginal, size_t endInOriginal) = 0;
  ...
  void increaseOffset(size_t increaseBy) {
    if (increaseBy > used()) {
      throw std::runtime_error(
          "The offset increase [" + std::to_string(increaseBy) + "] cannot be greater than the number of used bytes ["
          + std::to_string(used()) + "]");
    }
  ...
  void increaseEndOffset(size_t increaseBy) {
    if (increaseBy > remaining()) {
      throw std::runtime_error(
          ...
          + "] cannot be greater than the number of remaining bytes [" + std::to_string(remaining()) + "]");
    }
  ...
  [[nodiscard]] std::shared_ptr<Buffer> sliceRemaining() { return slice(mEnd, mCapacity); }
  ...
  [[nodiscard]] T* writePtr() {
    return reinterpret_cast<T*>(reinterpret_cast<uint8_t*>(base()) + endOffset());
  }
...
class UnownedBuffer : public Buffer {
 public:
  ...
  std::shared_ptr<Buffer> slice(size_t offsetInOriginal, size_t endInOriginal) override {
    const size_t newCapacity = endInOriginal - offsetInOriginal;

    if (newCapacity > capacity()) {
      throw std::invalid_argument(
          "Requested capacity [" + std::to_string(newCapacity) + "] exceeds actual capacity ["
          + std::to_string(capacity()) + "]");
    }

    return std::make_shared<UnownedBuffer>(mBuffer + offsetInOriginal, newCapacity, 0, 0);
  }
...
class OwnedBuffer : public Buffer {
 public:
  OwnedBuffer() : Buffer() {}
  OwnedBuffer(const std::shared_ptr<uint8_t>& buffer, size_t capacity, size_t offset, size_t end)
      : Buffer(capacity, offset, end), mBuffer(buffer) {}

  [[nodiscard]] uint8_t* base() override { return mBuffer.get(); }
  [[nodiscard]] const uint8_t* base() const override { return mBuffer.get(); }

  [[nodiscard]] std::shared_ptr<Buffer> slice(size_t offsetInOriginal, size_t endInOriginal) override {
    const size_t newCapacity = endInOriginal - offsetInOriginal;
    ...
    const auto slicedBuffer = std::shared_ptr<uint8_t>(mBuffer, mBuffer.get() + offsetInOriginal);
