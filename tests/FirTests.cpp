...
TEST(WhenThereAre4InputsWithDecimation2AndTwoInputCommits, ItProduces2CorrectOutputs) {
  ...
  Ref<IBuffer> buffer;
  buffer.reset(unwrap(fir->requestBuffer(0, inputDataCount * sizeof(cuComplex))));
  auto* gpuData = buffer->writePtr<cuComplex>();
  throwIfError(hostToDeviceMemCopier->copy(gpuData, cpuData1, sizeof(cpuData1)));
  throwIfError(fir->commitBuffer(0, sizeof(cpuData1)));

  buffer = unwrap(fir->requestBuffer(0, sizeof(cpuData2)));
  gpuData = buffer->writePtr<cuComplex>();
  throwIfError(hostToDeviceMemCopier->copy(gpuData, cpuData2, sizeof(cpuData2)));
  throwIfError(fir->commitBuffer(0, sizeof(cpuData2)));
  ...
  throwIfError(fir->readOutput(outputBuffers.data(), 1));
...
TEST(WhenTheFirstReadCantFitAllAvailableInputs, ItDoesntSkipAnyInputValues) {
  ...
  auto buffer = unwrap(fir->requestBuffer(0, sizeof(cpuData)));
  auto* gpuData = buffer->writePtr<cuComplex>();
  throwIfError(hostToDeviceMemCopier->copy(gpuData, cpuData, sizeof(cpuData)));
  throwIfError(fir->commitBuffer(0, sizeof(cpuData)));
  ...
  ConstRef<IBuffer> outputBuffer1 = unwrap(cudaBufferFactory->createBuffer(outputBuffer1Size));
  ConstRef<IBuffer> outputBuffer2 = unwrap(cudaBufferFactory->createBuffer(outputBuffer2Size));
  outputBuffers1.push_back(outputBuffer1.get());
  outputBuffers2.push_back(outputBuffer2.get());
  ...
  ConstRef<IBuffer> outputBuffer1Slice = unwrap(bufferSliceFactory->slice(outputBuffer1, 0, outputBuffer1Size));
  ConstRef<IBuffer> outputBuffer2Slice = unwrap(bufferSliceFactory->slice(outputBuffer2, 0, outputBuffer2Size));
  ...
  throwIfError(fir->readOutput(outputBuffers1.data(), 1));
  throwIfError(fir->readOutput(outputBuffers2.data(), 1));
  ...
  throwIfError(deviceToHostMemCopier
                   ->copy(hostOutputBuffer1, outputBuffer1->readPtr(), expectedOutputCount1 * sizeof(cuComplex)));
  throwIfError(deviceToHostMemCopier
                   ->copy(hostOutputBuffer2, outputBuffer2->readPtr(), expectedOutputCount2 * sizeof(cuComplex)));
