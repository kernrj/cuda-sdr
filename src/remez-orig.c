...
int CreateDenseGrid(int r, int numtaps, int numband, const double bands[],
 ...
 */
   ...
   for (band=0; band < numband; band++)
   {
      ...
      fprintf(stderr, "Other k = %d\n", k);
      for (i=0; i<k; i++)
      {
        if (j >= gridsize) {
          fprintf(stderr, "Too many: j (%d) >= gridsize (%d)\n",
                  j, gridsize);
          return -3; // too many
        }
        ...
        if (j < gridsize) {
          D[j] =
              des[2 * band] + i * (des[2 * band + 1] - des[2 * band]) / (k - 1);
          W[j] = weight[band];
          Grid[j] = lowf;
        }
         lowf += delf;
            ...
            int gridsize, double E[])
{
   ...
   int foundExtLength = gridsize; //2 * r;

/*
 * Allocate enough space for found extremals.
 */
   foundExt = (int *)malloc(foundExtLength * sizeof(int));
   k = 0;
 ...
 */
   for (i=1; i<gridsize-1; i++)
   {
    ...
    {
      free(foundExt);
      fprintf(stderr, "Too many extremal frequencies. k (%d) >= 2r (r=%d), gridsize %d\n",
              k, r, gridsize);
      return -3;
    }*/
    if (k < foundExtLength) {
   ...
   if (k >= foundExtLength) {
     free(foundExt);
     fprintf(stderr, "Too many extremal frequencies (new). k (%d) >= 2r (r=%d), gridsize %d\n",
             k, r, gridsize);
     return -3;
   }
 ...
 */
   ...
   if (((E[j]>0.0) && (E[j]>E[j-1])) ||
     ...
     {
       free(foundExt);
       fprintf(stderr, "Too many extremal frequencies (2nd). k (%d) >= 2r (r=%d)\n",
               k, r);
       return -3;
     }
     foundExt[k++] = j;
   ...
   {
     free(foundExt);
     fprintf(stderr, "Too few extremal frequencies. k (%d) < r+1 (r=%d)\n", k, r);
     return -2;
   }
...
int isDone(int r, int Ext[], double E[])
{
   ...
   if (max == 0.0f) {
     fprintf(stderr, "Not done, would divide by 0.\n");
     return 0; // avoid division by 0.
   }

   done = (((max-min)/max) < 0.0001);
   if (!done) {
     fprintf(stderr, "Not done because max (%f), min (%f), (max-min)/max >= 0.0001 (%f)\n",
             max, min, ((max-min)/max));
   }
   return done;
}
 ...
 */
   gridsize = 0;
   fprintf(stderr, "griddensity %d\n", *griddensity);
   fprintf(stderr, "r %d\n", r);
   fprintf(stderr, "gridsize = %d\n", gridsize);
   for (i=0; i < *numband; i++)
   {
      gridsize += (int)(2 * r * (*griddensity) * (bands[2*i+1] - bands[2*i]) + .5);
      fprintf(stderr, "gridsize += 2r * griddensity * (bandHi (%f) - bandLo(%f)) + 0.5\n",
              bands[2*i+1], bands[2*i]);
      fprintf(stderr, "sum %d\n", (int)(2 * r * (*griddensity) * (bands[2*i+1] - bands[2*i]) + .5));
      fprintf(stderr, "gridsize = %d\n", gridsize);
   }
   if (symmetry == NEGATIVE)
   {
      gridsize--;
      fprintf(stderr, "Symmetry is negative: gridsize-- = %d\n", gridsize);
   }
 ...
 */
   ...
   for (iter=0; iter<MAXITERATIONS; iter++)
   {
      ...
      if (err) {
        ...
        fprintf(stderr, "no error, gridsize=%d\n", gridsize);
      }
      ...
      if (isDone(r, Ext, E)) {
        ...
        fprintf(stderr, "Continuing after error\n");
      } else {
        fprintf(stderr, "Continuing, no error\n");
      }
   }

   if (iter == MAXITERATIONS) {
     if (err) {
       fprintf(stderr, "Quit with error after max iterations\n");
     } else {
       fprintf(stderr, "Quit with no error after max iterations\n");
     }
   }
