...
#include "cuda/fir.h"
#include "filters/AddConst.h"
#include "filters/CosineSource.h"
#include "filters/HackrfSource.h"
#include "filters/Int8ToFloat.h"
#include "filters/Magnitude.h"
...
int main(int argc, char** argv) {
  ...
  const auto decimation = static_cast<size_t>((floorf(maxRfSampleRate / audioSampleRate)));
  ...
  CosineSource cosineSource(rfSampleRate, centerFrequency - channelFrequency, cudaDevice, cudaStream);

  MultiplyCcc multiplyRfSourceByCosine(cudaDevice, cudaStream);

  const float gain = 1.0f;

  std::vector<float> lowPassTaps = generateLowPassTaps(gain, rfSampleRate, cutoffFrequency, transitionWidth);
  ...
  hackrfSource.setSampleCallback([&hackrfHostSamples,
                                  &convertHackrfInputToFloat,
                                  &cosineSource,
                                  &multiplyRfSourceByCosine](const int8_t* samples, size_t byteCount) {
    const size_t hackRfInputSampleCount = byteCount / 2;  // samples alternates between real and imaginary
    shared_ptr<Buffer> s8ToFloatInputCudaBuffer = convertHackrfInputToFloat.requestBuffer(0, byteCount);
    if (s8ToFloatInputCudaBuffer->remaining() < byteCount) {
      printf("Dropped\n");
    }

    size_t copyByteCount = min(byteCount, s8ToFloatInputCudaBuffer->remaining());
    ...
    memmove(s8ToFloatInputCudaBuffer->writePtr<int8_t>(), samples, copyByteCount);
    convertHackrfInputToFloat.commitBuffer(0, copyByteCount);

    const size_t multiplyInputCount = hackRfInputSampleCount * sizeof(cuComplex);
    shared_ptr<Buffer> rfMultiplyInput = multiplyRfSourceByCosine.requestBuffer(0, multiplyInputCount);
    shared_ptr<Buffer> cosineMultiplyInput = multiplyRfSourceByCosine.requestBuffer(1, multiplyInputCount);

    size_t multiplyInputSampleCount =
        min(rfMultiplyInput->remaining() / sizeof(cuComplex), cosineMultiplyInput->remaining() / sizeof(cuComplex));
  });
}
