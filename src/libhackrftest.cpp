...
static int rx_callback(hackrf_transfer *transfer) {
  ...
  printf("Buffer size %d %hhd %hhd %hhd %hhd\n", transfer->valid_length,
...
int main(int argc, char **argv) {
  ...
  SAFERF_RET(hackrf_device_list_open(deviceList, deviceIndex, &device),
  ...
  SAFERF_RET(hackrf_set_sample_rate(device, 256000),
             "Error setting sample rate");
  SAFERF_RET(hackrf_start_rx(device, rx_callback, nullptr),
