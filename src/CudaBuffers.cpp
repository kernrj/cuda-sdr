...
shared_ptr<Buffer> createAlignedBufferCuda(
    size_t bufferSize,
...
shared_ptr<Buffer> createAlignedBufferCudaHost(
    size_t bufferSize,
    size_t sizeAlignment) {
  static constexpr size_t addressAlignment = 128;
  ...
  return make_shared<OwnedBuffer>(
      std::shared_ptr<uint8_t>(
          std::shared_ptr<uint8_t>(
              rawBuffer,
              [](uint8_t* buffer) { cudaFreeHost(buffer); }),
          startAddress),
      usableLength,
      0,
      0);
...
void ensureMinCapacityAlignedCuda(
  ...
  const bool allocateNew =
      *buffer == nullptr || (*buffer)->capacity() < minAlignedSize;
  ...
  const shared_ptr<Buffer> newBuffer =
      createAlignedBufferCuda(minSize, alignment, cudaStream);
...
void ensureMinCapacityAlignedCudaHost(
  ...
  const shared_ptr<Buffer> newBuffer =
      createAlignedBufferCudaHost(minSize, alignment);
...
void appendToBufferCuda(
  ...
  if (count > buffer->remaining()) {
    throw invalid_argument(
        "Cannot copy more bytes [" + to_string(count) + "] than remain ["
        + to_string(buffer->remaining()));
  }
...
void readFromBufferCuda(
  ...
  if (count > buffer->used()) {
    throw invalid_argument(
        "Cannot copy more bytes [" + to_string(count) + "] than available ["
        + to_string(buffer->used()));
  }
...
void moveFromBufferCuda(
  ...
  if (count > src->used()) {
    throw invalid_argument(
        "Cannot copy more bytes [" + to_string(count)
        + "] than available in the source [" + to_string(src->used()));
  }

  if (count > dst->remaining()) {
    throw invalid_argument(
        "Cannot copy more bytes [" + to_string(count)
        + "] than remain in the destination [" + to_string(dst->remaining()));
  }
